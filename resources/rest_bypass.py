...
def hostnames(CIDR):
    ...
    # print bcolors.WARNING + "[!] If you want to continue undetected stop the
    # script and change /etc/hosts and /etc/hostname" + bcolors.ENDC
    print " "

    print "Searching for hostnames in %s\n" % CIDR
    try:
        subprocess.call(
            'sudo nbtscan -q %s | egrep "^[^A-Z]*[A-Z]{5,15}[^A-Z]*$" | awk {\'print $2\'} > ../Results/hostnames' % CIDR, shell=True)
        subprocess.call(
            "sudo sort ../Results/hostnames | uniq > ../Results/unique_hosts", shell=True)
...
def static_bypass():
    ...
    subprocess.call(
        "sudo netdiscover -i eth0 -P -l ./resources/discover | grep -P -o \'([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+).*? ' | grep -P -o \'[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' > ../Results/ips_discovered", shell=True)
...
def create_subnet():
    ...
    subprocess.call(
        "sudo sort ../Results/subnets | uniq > ../Results/unique_subnets", shell=True)
...
def set_static(CIDR):
    ...
    subprocess.call(
        "sudo sort ../Results/CIDR | uniq > ../Results/unique_CIDR", shell=True)
    subprocess.call("sudo rm ../Results/CIDR", shell=True)
    subprocess.call(
        "sudo netdiscover -i eth0 -P -l ./resources/discover | grep -P -o \'([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+).*? ' | grep -P -o \'[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' > ../Results/used_ips", shell=True)
    ...
    with open('../Results/statics', 'r') as statics:
        ...
        for i in range(0, line_count):
            ...
            subprocess.call(
                ["ifconfig", "eth0", static.strip(), "netmask", netmask.strip()])
...
def macbypass(unique_CIDR):
    ...
    subprocess.call(
        "sudo netdiscover -i eth0 -P -r  %s | grep -o -E /'([[:xdigit:]]{1,2}:){5}[[:xdigit:]]{1,2}/' > ../Results/macs_discovered" % unique_CIDR, shell=True)

    subprocess.call(
        "sudo sort ../Results/macs_discovered | uniq > ../Results/unique_macs", shell=True)
...
def nacbypass(unique_CIDR):
    ...
    subprocess.call(
        "sudo tcpdump -i eth0 -vvv port 137 -c 10 > ../Results/network_traffic")
    search = ".netbios"
    subprocess.call(
        "grep %s ../Results/network_traffic > ../Results/ips_found" % search, shell=True)
    ip_net = []
    ...
    search = "Name="
    subprocess.call(
        "grep %s ../Results/network_traffic > ../Results/names_found " % search, shell=True)
    n = []
    ...
    subprocess.call(
        "sudo netdiscover -P -r %s | awk {'print $1,$2'} > ../Results/ips_macs" % unique_CIDR, shell=True)
        ...
        for name in Fname:
            if word == name:
                ...
                with open('/etc/hostname', 'w') as hostname:
                    ...
                    subprocess.call('sudo systemctl daemon-reload', shell=True)
                subprocess.call('sudo ifdown eth0', shell=True)
                subprocess.call('sudo macchanger -m %s eth0' %
                                Fmac[ic], shell=True)
                subprocess.call('sudo ifup eth0', shell=True)
