...
def shares_enum():
    ...
    subprocess.call(
        "sudo sort ../Results/windows | uniq > ../Results/win_hosts", shell=True)
    ...
    with open('../Results/win_hosts', 'r') as hosts:
        ...
        for host in hosts:
            ...
            nm = nmap.PortScanner()
            nm.scan(
                hosts=host, arguments='-Pn -T4 --script smb-enum-shares -p445 --open -o ../Results/shares')
...
def smb_users():
    ...
    with open('../Results/win_hosts') as hosts:
        for host in hosts:
            ...
            nm = nmap.PortScanner()
            nm.scan(
                hosts=host, arguments='-Pn -T4 -sU -sS --script smb-enum-users -p U:137,T:139 --open -o ../Results/smb_users')
...
def http_title_enum():
    ...
    subprocess.call(
        "sudo cat ../Results/webserver* > ../Results/webs", shell=True)
    ...
    subprocess.call(
        "sudo sort ../Results/webs | uniq > ../Results/web_hosts", shell=True)
    ...
    with open('../Results/web_hosts') as webs:
        for host in webs:
            ...
            nm = nmap.PortScanner()
            nm.scan(
                hosts=host, arguments='-Pn -T4 -sC -p80,8080,443,4443,8081,8181,9090 --open -o ../Results/http_titles')
...
def nfs_enum():
    ...
    subprocess.call(
        "sudo sort ../Results/nfs | uniq > ../Results/nfs_hosts", shell=True)
    with open('../Results/nfs_hosts') as shares:
        for share in shares:
            print "[*] Enumerating NFS Shares on %s" % share.strip()
            nm = nmap.PortScanner()
            nm.scan(
                hosts=share, arguments='-Pn -sV -T4 --script afp-showmount -p111 --open -o ../Results/nfs_enum')
...
def mysql_enum():
    ...
    subprocess.call(
        "sudo sort ../Results/mysql | uniq > ../Results/mysql_hosts", shell=True)
    with open('../Results/mysql_hosts') as dbs:
        for db in dbs:
            print "[*] Enumerating MYSQL DB on %s" % db.strip()
            nm = nmap.PortScanner()
            nm.scan(
                hosts=db, arguments='-Pn -T4 -sV --script mysql-enum -p3306 --open -o ../Results/mysql_enum')
...
def mssql_enum():
    ...
    subprocess.call(
        "sudo sort ../Results/mssql | uniq > ../Results/mssql_hosts", shell=True)
    with open('../Results/mssql_hosts') as dbs:
        for db in dbs:
            print "[*] Enumerating MSSQL DB on %s" % db.strip()
            nm = nmap.PortScanner()
            nm.scan(
                hosts=db, arguments='-Pn -T4 -sV --script ms-sql-info -p1433 --open -o ../Results/mssql_enum')
...
def ftp_enum():
    ...
    subprocess.call(
        "sudo sort ../Results/ftp | uniq > ../Results/ftp_hosts", shell=True)
    with open('../Results/ftp_hosts') as ftps:
        for ftp in ftps:
            print "[*] Enumerating FTP on %s" % ftp.strip()
            nm = nmap.PortScanner()
            nm.scan(
                hosts=ftp, arguments='-Pn -T4 -sV --script ftp-anon -p22 --open -o ../Results/ftp_enum')
...
def snmp_enum():
    ...
    subprocess.call(
        "sudo sort ../Results/snmp | uniq > ../Results/snmp_hosts", shell=True)
    with open('../Results/snmp_hosts') as snmps:
        for snmp in snmps:
            print "[*] Enumerating SNMP on %s" % snmp.strip()
            nm = nmap.PortScanner()
            nm.scan(
                hosts=snmp, arguments='-Pn -T4 -sV -p161 --open -o ../Results/snmp_enum')
...
def pcap_parser():
    ...
    subprocess.call(
        "sudo python ../Tools/net-creds/net-creds.py -p ../Results/capture.pcap > ../Results/pcap_results", shell=True)
...
def poison():
    ...
    subprocess.call('sudo python Responder.py -I eth0',
                    cwd=r'../Tools/Responder/', shell=True)
